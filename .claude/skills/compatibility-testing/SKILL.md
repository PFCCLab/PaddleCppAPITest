...
**覆盖率计算陷阱与规避：**
- **不要为了骗覆盖率而在测试中堆砌无效对象实例化**：之前的测试中，为了凑对 `torch::Library` 和注册表的测试，出现了只实例化而不调用方法的情况。覆盖率脚本是基于 `.方法名(` / `::方法名` 等正则计算的，只有真实涉及方法调用才会算入覆盖率。
- **避免硬编码兼容宏从而忽略真实 API**：对于测试一些宏背后的类方法或内部函数（如 `FunctionArgs`, `FunctionResult`, `CppFunction`），需真实模拟数据去调用它门原本对外开放的公共接口，如 `.call()`, `.get()`, `.has_value()`。
- **模板方法与命名空间匹配**：写单测时如果要触发覆盖率统计的正则，尽可能使用带参数的具体实现格式。遇到复杂的工具接口（例如 `ClassRegistry`，`invoke_function` 等），可以直接构造虚拟对象 `CustomClassHolder` 进行显式的方法签名匹配和调用，切忌遗漏。
- **禁止使用条件编译区分 API**：测试 Paddle 兼容层与 PyTorch 的行为一致性时，**禁止**使用 `#if USE_PADDLE_API` 或类似的条件编译来区分同一个方法的蛇形/驼峰写法。这样做会导致测试代码无法同时验证两个框架的行为是否一致。应直接调用目标框架的实际 API，确保测试的是运行时行为而非编译期分支。
