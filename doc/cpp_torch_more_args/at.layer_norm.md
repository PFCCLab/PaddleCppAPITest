...
| normalized_shape | begin_norm_axis | ⚠️ 类型与语义差异，`normalized_shape` 是尾部维度形状列表，`begin_norm_axis` 是轴索引，二者不可直接互换，调用端需转换。 默认值不同：PyTorch 无默认值，Paddle 默认 `begin_norm_axis=1`。 |
